...
(
	SELECT * 
	FROM sys.tables	
	WHERE name = N'ProductCategories'
)
...
--Clear ProductCategoriesTableInsert stored procedure
IF OBJECT_ID('dbo.ProductCategoriesTableInsert', 'P') IS NOT NULL
	DROP PROCEDURE dbo.ProductCategoriesTableInsert
GO

--Create ProductCategoriesTableInsert stored procedure
CREATE PROCEDURE dbo.ProductCategoriesTableInsert
	AS
	BEGIN TRY
		IF EXISTS(SELECT 1 FROM BusinessDatabase.dbo.ProductCategories)
			BEGIN
				SELECT 'ProductCategories table is populated.' AS ProductCategoriesTableStatus
			END
		ELSE
			BEGIN
				SELECT 'ProductCategories table is not populated - Inserting values.' AS ProductCategoriesTableStatus
					INSERT INTO BusinessDatabase.dbo.ProductCategories(ProductCategoryName, ParentCategory)
					VALUES('Electronics', NULL),
						  ...
						  ('Canin60D', 2),
						  ('MakBookBro', 3),
						  ('Household Goods', NULL),
						  ('Bathroom Items', 7),
						  ('Rubber Duck', 8)
			END
	END TRY
	BEGIN CATCH
		SELECT 'Error in ProductCategories table insert.' 
	END CATCH
GO

--Run ProductCategoriesTableInsert stored procedure
EXECUTE dbo.ProductCategoriesTableInsert
GO
...
SELECT * FROM BusinessDatabase.dbo.ProductCategories
...
--Return item's parent categories; display categoryid, name, and parent category
WITH ParentCategories(ProductCategoryID, ProductCategoryName, ParentCategory) 
AS(
	SELECT C.ProductCategoryID, C.ProductCategoryName, C.ParentCategory
	FROM BusinessDatabase.dbo.ProductCategories C
