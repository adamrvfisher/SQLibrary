		...
		ELSE
			BEGIN
				SELECT 'Customers table is not populated - Inserting values.' AS CustomersTableStatus
					INSERT INTO BusinessDatabase.dbo.Customers(FirstName, LastName, DateOfBirth, CreditLimit, CompanyName, TownID, CreatedDate)
					VALUES('Yvonne', 'McKay', '1984-05-25', 9000, NULL, NULL, GETDATE()),
						  ('Jossef', 'GoldBerg', '1995-07-04', 5500, NULL, NULL, GETDATE())
			END
	END TRY
	BEGIN CATCH
		...
		ELSE
			BEGIN
				SELECT 'Employees table is not populated - Inserting values.' AS EmployeesTableStatus
					INSERT INTO Employees(EmployeeID, FirstName, LastName, HireDate, HourlyPay, IsDeleted)
					VALUES('1', 'Blanche', 'Guzman', '2019-07-14', 106, 0),
						  ('2', 'Jarrod', 'Booth', '2020-05-25', 55 , 0)
			END
	END TRY
	BEGIN CATCH
		...
		ELSE
			BEGIN
				SELECT 'UserLogin table is not populated - Inserting values.' AS UserLoginTableStatus
					INSERT INTO BusinessDatabase.dbo.UserLogin(UserName, PassCode, IsDeleted)
					VALUES('BlanchGuzman', 'iWorkAllDay', 0),
						  ('JarrodBooth', 'ReallyBadPassword', 1)
			END
	END TRY
	BEGIN CATCH
